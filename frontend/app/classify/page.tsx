...
const Classify: React.FC = () => {
  ...
  const tagOptions = ['Important', 'Promotions', 'Social', 'Marketing', 'Spam', 'General'];
  ...
  }, [status, session, numEmails, selectedTag]); 
    ...
    try {
      ...
      if (apiKey) {
        ...
        const filteredEmails = selectedTag
          ? classifiedEmails.filter((email : any) => email.category === selectedTag)
          : classifiedEmails;
  ...
  return (
    <Container maxWidth="md" style={{ marginTop: '2rem' }}>
      ...
      {/* Added dropdown menu for selecting tags */}
      <FormControl fullWidth variant="outlined" style={{ marginTop: '1rem' }}>
        <InputLabel>Select tag to display</InputLabel>
        <Select
          value={selectedTag}
          onChange={(e) => setSelectedTag(e.target.value)}
          label="Select tag to display"
        >
          ...
          <Grid item xs={12} key={email.id}>
            <Card>
              ...
              <CardActions>
                <Typography
                  variant="button"
                  style={{ marginLeft: 'auto', color: getCategoryColor(email.category) }}
                >
                  {email.category}
                </Typography>
              </CardActions>
